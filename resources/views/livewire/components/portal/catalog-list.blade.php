<section x-data="{ tab: 'collections' }">
    ...
    <div x-show="tab == 'collections'"> {{-- Коллекции --}}
        <div class="flex gap-8 flex-wrap justify-between mb-4">
            @foreach($collections as $collection)
                <x-cards.collection :collection="$collection"/>
            @endforeach
        </div>
        @if($orig_collections->count() <= $collection_limit)
            <p class="text-lg">Все коллекции ({{$orig_collections->count()}}) загружены</p>
        @else
            <p wire:click="loadMoreCollections" class="cursor-pointer text-lg">Загрузить еще</p>
        @endif
    ...
    <div x-show="tab == 'products'"> {{-- Товары --}}
        ...
        @if($totalProducts <= $product_limit)
            <p class="text-lg">Все товары ({{$totalProducts}}) загружены</p>
        @else
            <p wire:click="loadMoreProducts" class="cursor-pointer text-lg">Загрузить еще</p>
        @endif
    </div>
</section>
